...
#include UE_INLINE_GENERATED_CPP_BY_NAME(FloodComponent2D)

UFloodComponent2D::UFloodComponent2D(const FObjectInitializer& ObjectInitializer) 
...
bool UFloodComponent2D::ValidateJumpFloodRenderTargets()
{
	if ((RTA == nullptr) 
		|| (RTB == nullptr) 
		...
		|| (RTA->SizeY != RTB->SizeY))
	{
		UE_LOG(LogTemp, Error, TEXT("Invalid Render Target used in Jump Flood Component 2D."));
		return false;
	}
...
bool UFloodComponent2D::ValidateJumpFloodRequirements()
{
	if (JumpStepMID == nullptr)
	{
		UE_LOG(LogTemp, Error, TEXT("Invalid Jump Step material used in Jump Flood Component 2D."));
		return false;		
	}

	if (UseBlur && (BlurEdgesMID == nullptr))
	{
		UE_LOG(LogTemp, Error, TEXT("Invalid Blur Edges material used in Jump Flood Component 2D."));
		return false;
	}

	if (FindEdgesMID == nullptr)
	{
		UE_LOG(LogTemp, Error, TEXT("Invalid Find Edges material used in Jump Flood Component 2D."));
		return false;
	}
...
void UFloodComponent2D::JumpFlood(UTextureRenderTarget2D* SeedRT, float SceneCaptureZ, FLinearColor Curl, bool UseDepth, float ZLocationT)
{
	CreateMIDs();
	
	if (!ValidateJumpFloodRequirements())
	{
		UE_LOG(LogTemp, Error, TEXT("Invalid setup for Jump Flood Component 2D. Aborting JumpFlood."));
		return;
	}
...
bool UFloodComponent2D::CreateMIDs()
{
	...
	{
		UE_LOG(LogTemp, Error, TEXT("Invalid JumpFlood materials."));
		return false;
	}
...
void UFloodComponent2D::AssignRenderTargets(UTextureRenderTarget2D* InRTA, UTextureRenderTarget2D* InRTB)
{
	RTA = InRTA;
	RTB = InRTB;
}

UTextureRenderTarget2D* UFloodComponent2D::SingleJumpStep()
{
	if (JumpStepMID == nullptr)
	{
		UE_LOG(LogTemp, Error, TEXT("Invalid Jump Step material for Jump Flood Component 2D. Aborting SingleJumpStep."));
		return nullptr;
	}

	if (!ValidateJumpFloodRenderTargets())
	{
		UE_LOG(LogTemp, Error, TEXT("Invalid Render Targetfor Jump Flood Component 2D. Aborting SingleJumpStep."));
		return nullptr;
	}
...
UTextureRenderTarget2D* UFloodComponent2D::FindEdges(UTextureRenderTarget2D* InSeed, float InCaptureZ, FLinearColor InCurl, bool InUseDepth, float InZLocation)
{
	if (FindEdgesMID == nullptr)
	{
		UE_LOG(LogTemp, Error, TEXT("Invalid Find Edges material for Jump Flood Component 2D. Aborting FindEdges."));
		return nullptr;
	}

	if (!ValidateJumpFloodRenderTargets())
	{
		UE_LOG(LogTemp, Error, TEXT("Invalid Render Targetfor Jump Flood Component 2D. Aborting FindEdges."));
		return nullptr;
	}
...
void UFloodComponent2D::FindEdges_Debug(UTextureRenderTarget2D* InSeed, float InCaptureZ, FLinearColor InCurl, UTextureRenderTarget2D* inDest, float InZOffset)
{
	if (FindEdgesMID == nullptr)
	{
		UE_LOG(LogTemp, Error, TEXT("Invalid Find Edges material for Jump Flood Component 2D. Aborting FindEdges_Debug."));
		return;
	}

	if (!ValidateJumpFloodRenderTargets())
	{
		UE_LOG(LogTemp, Error, TEXT("Invalid Render Targetfor Jump Flood Component 2D. Aborting FindEdges_Debug."));
		return;
	}
...
UTextureRenderTarget2D* UFloodComponent2D::SingleBlurStep()
{
	if (BlurEdgesMID == nullptr)
	{
		UE_LOG(LogTemp, Error, TEXT("Invalid Blur Edges material for Jump Flood Component 2D. Aborting SingleBlurStep."));
		return nullptr;
	}

	if (!ValidateJumpFloodRenderTargets())
	{
		UE_LOG(LogTemp, Error, TEXT("Invalid Render Targetfor Jump Flood Component 2D. Aborting SingleBlurStep."));
		return nullptr;
	}
...
UTextureRenderTarget2D* UFloodComponent2D::PingPongSource(int32 Offset) const
{
...
UTextureRenderTarget2D* UFloodComponent2D::PingPongTarget(int32 Offset) const
{

...
#define LOCTEXT_NAMESPACE "TestModeModule"

//DEFINE_LOG_CATEGORY(LogRoadEditor);

void FTestModeModule::StartupModule()
{
	FTestModeStyle::Initialize();
	...
	GEngine->OnLevelActorAdded().AddRaw(this, &FTestModeModule::OnLevelActorAddedToWorld);
	...
	ActorPickerHandle = FMoKuEditorActorPicker::Get().OnActorPicked().AddRaw(this, &FTestModeModule::OnEditorActorPicked);
}

void FTestModeModule::OnEditorActorPicked(AActor* PickedActor)
{
	...
	UE_LOG(LogTemp, Log, TEXT("[TestMode] OnActorPicked: %s (%s)"),
		*PickedActor->GetActorLabel(),
		*PickedActor->GetClass()->GetName());
}

void FTestModeModule::ShutdownModule()
{
	...
	FTestModeEditorModeCommands::Unregister();
	...
	FTestModeStyle::Shutdown();

}


void FTestModeModule::OnLevelActorAddedToWorld(AActor* Actor)
{
	if (!Actor)return;
...
IMPLEMENT_MODULE(FTestModeModule, TestMode)

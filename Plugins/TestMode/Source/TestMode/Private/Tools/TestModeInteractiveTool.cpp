#include "TestModeInteractiveTool.h"
...
#define LOCTEXT_NAMESPACE "UTestModeInteractiveTool"

UInteractiveTool* UTestModeInteractiveToolBuilder::BuildTool(const FToolBuilderState & SceneState) const
{
	UTestModeInteractiveTool* NewTool = NewObject<UTestModeInteractiveTool>(SceneState.ToolManager);
	NewTool->SetWorld(SceneState.World);
	NewTool->SetGizmoManager(SceneState.GizmoManager);
	return NewTool;
}
void UTestModeInteractiveTool::SetWorld(UWorld* World)
{
	check(World);
	this->TargetWorld = World;
}
void UTestModeInteractiveTool::Setup()
{
	...
	GEditor->OnLevelActorAdded().AddUObject(this, &UTestModeInteractiveTool::Test);
...
void UTestModeInteractiveTool::SetUpBehaviors()
{
...
void UTestModeInteractiveTool::Shutdown(EToolShutdownType ShutdownType)
{
...
FInputRayHit UTestModeInteractiveTool::ShouldRespondToMouseWheel(const FInputDeviceRay& CurrentPos)
{
...
void UTestModeInteractiveTool::OnUpdateModifierState(int ModifierID, bool bIsOn)
{
...
FInputRayHit UTestModeInteractiveTool::CanBeginClickDragSequence(const FInputDeviceRay& PressPos)
{
	FVector Temp;
	FInputRayHit Result = FindRayHit(PressPos.WorldRay, Temp);
	return Result;
}
FInputRayHit UTestModeInteractiveTool::BeginHoverSequenceHitTest(const FInputDeviceRay& PressPos)
{
	return CanBeginClickDragSequence(PressPos);
}
void UTestModeInteractiveTool::OnClickPress(const FInputDeviceRay& PressPos)
{
	//功能完成代码待整理
...
void UTestModeInteractiveTool::DrawHUD(FCanvas* Canvas, IToolsContextRenderAPI* RenderAPI)
{
...
void UTestModeInteractiveTool::UpdateSplineRoad(USplineComponent* IntersectCurve, USplineComponent* InEditingCurve, const FVector& InIntersectionPos,bool NeedInvert)
{
...
void UTestModeInteractiveTool::UpdateRoadJunctionInfo
(
...
FCornerInfo UTestModeInteractiveTool::UpdateJunctionSideInfo(USplineComponent* IntersectComp, TArray<FVector> IntersectionPosList, EIntersectionState IntersectionState)
{
...
FVector UTestModeInteractiveTool::UpdateSplineRoad(AMoKuEditBaseActor* InHitActor, AMoKuEditIntersectionActor* InJunctionActor,bool NeedInvert)
{
...
void UTestModeInteractiveTool::UpdateSplineAndJunction
(
	FCurvesRoadOp* InRoadOp, 
	AMoKuEditIntersectionActor*& InJunctionActor,
...
void UTestModeInteractiveTool::AddSplinePoint(const FVector& InHitPos, const FRotator& InHitRot, bool InteractiveGizmo,const FVector& InTangent,UInteractiveGizmo* InGizmoInfo)
{
	...
	if (SplineCurvesProperty)
	{
		...
		// 通知拥有这个组件的 Actor（也就是你的 MoKuEditSplineActor）
...
void UTestModeInteractiveTool::OnMouseWheelScrollUp(const FInputDeviceRay& CurrentPos)
{
...
void UTestModeInteractiveTool::GetSelectedAsset(int Index)
{
	if (!CornerDataTable)return;
	TArray<FName> RowNames = CornerDataTable->GetRowNames();
...
void UTestModeInteractiveTool::OnMouseWheelScrollDown(const FInputDeviceRay& CurrentPos)
{
	if (!CornerDataTable)return;
	TArray<FName> RowNames = CornerDataTable->GetRowNames();
	int32 TotalRows = RowNames.Num();
...
void UTestModeInteractiveTool::GetSplitIntersectionData(AActor* InSplitActor,const FVector& InWorldPos)
{
	if (InSplitActor)
	{
...
void UTestModeInteractiveTool::UpdateEditState(bool InEditing)
{
	bIsEditing = InEditing;
}
void UTestModeInteractiveTool::OnClickDrag(const FInputDeviceRay& DragPos)
{	
}
void UTestModeInteractiveTool::OnClickRelease(const FInputDeviceRay& ReleasePos)
{

	GEditor->EndTransaction();

}
void UTestModeInteractiveTool::OnTerminateDragSequence()
{

}
void UTestModeInteractiveTool::OnPropertyModified(UObject* PropertySet, FProperty* Property)
{
	UE_LOG(LogTemp,Warning,TEXT("ParmChanged"))
}
FInputRayHit UTestModeInteractiveTool::FindRayHit(const FRay& WorldRay, FVector& HitPos)
{
	FCollisionObjectQueryParams QueryParams(FCollisionObjectQueryParams::AllObjects);
	FHitResult Result;
...
void UTestModeInteractiveTool::Render(IToolsContextRenderAPI* RenderAPI)
{
...
void UTestModeInteractiveTool::InvertPreviewMesh()
{
...
void UTestModeInteractiveTool::SwitchInsertCrossState()
{
	if (ActiveActor)
	{
		if (bIsInsertCross)
...
void UTestModeInteractiveTool::OnBeginHover(const FInputDeviceRay& DevicePos)
{

}
bool UTestModeInteractiveTool::OnUpdateHover(const FInputDeviceRay& DevicePos)
{
	const FName TraceTagName = FName("WorldCollision");
	FCollisionObjectQueryParams QueryParams(FCollisionObjectQueryParams::AllObjects);
	FHitResult Result;
...
void UTestModeInteractiveTool::OnFinishedSplineEdit()
{
	if(ActiveActor)
	{
		if (GetEditState())
...
void UTestModeInteractiveTool::PostEditUndo()
{
...
void UTestModeInteractiveTool::RefreshGizmoInfo()
{
	if (ActiveActor)
	{
...
void USplineFinishedBehavior::Initialize(UTestModeInteractiveTool* InSplineEditTool)
{
